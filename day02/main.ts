...
type Level = {
    level: number[],
    levelType: "increasing" | "decreasing" | null
    safe: boolean
}

const verifiers = {
    "increasing": (a: number, b: number) => (b - a) > 0 && (b - a) <= 3 ? true : false,
    "decreasing": (a: number, b: number) => (a - b) > 0 && (a - b) <= 3 ? true : false,
}

const levels: Level[] = input.split("\n").map(line => {
    const result: Level = {
        level: [],
        levelType: null,
        safe: false
...
const safeLevels2 = levels.filter(obj => {
    ...
    for(let i = 0;i < obj.level.length;i++){
        ...
        let f:"increasing" | "decreasing" | null = null;
