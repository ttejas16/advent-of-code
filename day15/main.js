...
function walkRobot(x, y, dirVector) {
    const start = [x, y];
    const newStart = [-1, -1];
...
function canBoxBeMoved(x, y, dirVector) {
    const boxChars = [];
    if (warehouse[x][y] == '[') {
        boxChars.push([x, y]);
        boxChars.push([x, y + 1]);
...
function movePositions(positions, dirVector) {
    const newStart = [-1, -1];
