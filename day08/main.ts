...
function manhattanDistance(first: number[], second: number[]): number[] {
    return [Math.abs(first[0] - second[0]), Math.abs(first[1] - second[1])]
}

function getAntinodes(a: number[], b: number[], rows: number, cols: number) {
    const result: number[][] = [];

    const bShifted: number[] = [b[0] - a[0], b[1] - a[1]];
    const bReflection: number[] = bShifted.map(n => -n);
    const r1 = [
        bReflection[0] + a[0], bReflection[1] + a[1]
    ]
    result.push(r1);

    const aShifted: number[] = [a[0] - b[0], a[1] - b[1]];
    const aReflection: number[] = aShifted.map(n => -n);
    const r2 = [
